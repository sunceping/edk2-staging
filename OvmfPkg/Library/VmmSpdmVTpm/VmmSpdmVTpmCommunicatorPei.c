  ...
  )
{
  ...
  if (sizeof (WorkArea->TdxWorkArea.SecTdxWorkArea.SpdmSecureSessionInfo) < VTPM_SECURE_SESSION_INFO_TABLE_SIZE){
    DEBUG((
      DEBUG_ERROR, 
      "%a: The size of SpdmSecureSessionInfo %d in WorkArea is less than VTPM_SECURE_SESSION_INFO_TABLE_SIZE %d\n",
      __FUNCTION__,
      sizeof (WorkArea->TdxWorkArea.SecTdxWorkArea.SpdmSecureSessionInfo),
      VTPM_SECURE_SESSION_INFO_TABLE_SIZE
      ));
    return EFI_UNSUPPORTED;
  }
...
/**
 * Check if a SecuredSpdmSession is established by finding 
 * the valid Spdm Secure session info in WorkArea.
 *
 * @return EFI_STATUS
 */
EFI_STATUS
